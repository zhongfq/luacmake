...
const static char *sproto_src = R"~~~(
...
function sproto.parse(ptext)
	local parser = require "sproto.parser"
...
static const char *sprotoparser_src = R"~~~(
...
extern "C" {
LUAMOD_API int luaopen_sproto_parser(lua_State *L) {
    luaL_loadstring(L, sprotoparser_src);
    lua_call(L, 0, 1);
    return 1;
}

LUAMOD_API int luaopen_sproto(lua_State *L) {
    luaL_loadstring(L, sproto_src);
    lua_call(L, 0, 1);
    return 1;
}
}